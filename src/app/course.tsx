...
export default function CourseScreen() {
    ...
    const renderSupport = ({ item }) => (
        <TouchableOpacity style={[styles.itemCard, { backgroundColor: colors.card, borderColor: colors.border }]} onPress={() => item.file_url ? window.open(`${API_URL.replace('/api','')}${item.file_url}`, '_blank') : null}>
            ...
            <View style={styles.itemActions}>
                ...
                <TouchableOpacity onPress={() => item.file_url ? Alert.alert('Lien', `${API_URL.replace('/api','')}${item.file_url}`) : null} style={styles.itemBtn}><Text>🔗</Text></TouchableOpacity>
            </View>
        </TouchableOpacity>
    );
    ...
    const renderNote = ({ item }) => (
        <View style={[styles.itemCard, { backgroundColor: colors.card, borderColor: colors.border, borderLeftWidth: 3, borderLeftColor: colors.primary }]}>
            ...
            <View style={styles.itemInfo}>
                <Text style={[styles.itemTitle, { color: colors.text }]} numberOfLines={1}>{item.title}</Text>
                <Text style={[styles.itemSubtitle, { color: colors.textSec }]} numberOfLines={2}>{item.content || 'Note vide'}</Text>
            </View>
            <View style={styles.itemActions}>
                <TouchableOpacity onPress={() => deleteItem(item.id)} style={styles.itemBtn}><Text>🗑️</Text></TouchableOpacity>
            </View>
                    ...
                    <View style={[styles.modalCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                        ...
                        <View style={styles.typeSelector}>
                            {['pdf', 'link', 'note'].map(type => (
                                <TouchableOpacity key={type} style={[styles.typeBtn, addType === type && { backgroundColor: colors.primary + '30', borderColor: colors.primary }]} onPress={() => setAddType(type)}>
                                    <Text style={{ color: addType === type ? colors.primary : colors.textSec }}>{type === 'pdf' ? '📄 PDF' : type === 'link' ? '🔗 Lien' : '📝 Note'}</Text>
                                </TouchableOpacity>
                            ))}
                        </View>
                        ...
                        {addType === 'pdf' && (
                            <>
                                ...
                                <TouchableOpacity style={[styles.btn, { backgroundColor: colors.primary }]} onPress={async () => {
                                    ...
                                    loadCourse();
                                }}><Text style={{ color: '#fff', textAlign: 'center' }}>Ajouter PDF</Text></TouchableOpacity>
                            </>
                        )}
                        ...
                        {addType === 'link' && (
                            <>
                                ...
                                <TouchableOpacity style={[styles.btn, { backgroundColor: colors.primary }]} onPress={async () => {
                                    ...
                                    loadCourse();
                                }}><Text style={{ color: '#fff', textAlign: 'center' }}>Ajouter Lien</Text></TouchableOpacity>
                            </>
                        )}
                        ...
                        {addType === 'note' && (
                            <TouchableOpacity style={[styles.btn, { backgroundColor: colors.primary }]} onPress={() => { setAddModalVisible(false); setNotebookVisible(true); }}>
                                <Text style={{ color: '#fff', textAlign: 'center' }}>📝 Ouvrir le cahier</Text>
                            </TouchableOpacity>
                        )}
            ...
            <Modal visible={notebookVisible} transparent animationType="slide">
                <View style={[styles.notebookContainer, { backgroundColor: colors.bg }]}>
                    <View style={styles.notebookHeader}>
                        <TouchableOpacity onPress={() => setNotebookVisible(false)}><Text style={{ color: colors.primary }}>Annuler</Text></TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    list: { padding: 12 },
    itemCard: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, borderRadius: 14, marginBottom: 8, borderWidth: 1 },
    ...
    notebookHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.1)' },
    noteTitleInput: { fontSize: 18, fontWeight: '700', padding: 16, borderBottomWidth: 2 },
    noteContentInput: { flex: 1, fontSize: 16, padding: 16, lineHeight: 28 },
});