...
export default function VaultScreen() {
    ...
    const unlock=async()=>{if(!pass.trim())return;const r=await apiFetch(`${API_URL}/vault/verify`,{method:'POST',body:JSON.stringify({matricule:u.matricule,password:pass})});const d=await r.json();if(d.success){slocked(false);loadVault()}};
    const loadVault=async()=>{const r=await apiFetch(`${API_URL}/vault/${u.matricule}`);const d=await r.json();if(d.success){scourses(d.courses);snotes(d.notes)}};
    const ri=(t,id)=>{setSelType(t);setSelId(id);setConfirmRestore(true)};
    const doRestore=async()=>{setConfirmRestore(false);await apiFetch(`${API_URL}/toggle-visibility/${selType}/${selId}`,{method:'POST'});loadVault()};
    const di=(t,id)=>{setSelType(t);setSelId(id);setConfirmDelete(true)};
    const doDelete=async()=>{setConfirmDelete(false);if(selType==='course'){await apiFetch(`${API_URL}/courses/${selId}`,{method:'DELETE'})}else{await apiFetch(`${API_URL}/notes/${selId}`,{method:'DELETE'})}loadVault()};
    ...
    if(locked){
        return(
            <View style={[ss.ct,{backgroundColor:cl.bg,justifyContent:'center',alignItems:'center',padding:30}]}>
                ...
                <TouchableOpacity onPress={()=>router.back()} style={{marginTop:16}}><Text style={{color:cl.ts,fontSize:14}}><FontAwesome5 name="arrow-left" size={12} color={cl.ts}/> Retour</Text></TouchableOpacity>
            </View>
        );
    }
    ...
    return(
        <View style={[ss.ct,{backgroundColor:cl.bg}]}>
            ...
            <View style={[ss.hd,{backgroundColor:cl.cd,borderBottomColor:cl.bd}]}><TouchableOpacity onPress={()=>router.back()} style={ss.hb}><FontAwesome5 name="arrow-left" size={18} color={cl.pr}/></TouchableOpacity><Text style={[ss.ht,{color:cl.tx}]}><FontAwesome5 name="lock" size={16} color={cl.pr}/> Coffre-fort</Text><View style={ss.hb}/></View>
            <View style={ss.tabs}>
                <TouchableOpacity style={[ss.tab,tab==='courses'&&{backgroundColor:cl.pr}]} onPress={()=>stab('courses')}><Text style={[ss.tt,{color:tab==='courses'?'#fff':cl.ts}]}><FontAwesome5 name="book" size={12} color={tab==='courses'?'#fff':cl.ts}/> Cours ({courses.length})</Text></TouchableOpacity>
                <TouchableOpacity style={[ss.tab,tab==='data'&&{backgroundColor:cl.pr}]} onPress={()=>stab('data')}><Text style={[ss.tt,{color:tab==='data'?'#fff':cl.ts}]}><FontAwesome5 name="file-alt" size={12} color={tab==='data'?'#fff':cl.ts}/> Données ({notes.length})</Text></TouchableOpacity>
            </View>
            {tab==='courses'&&(<FlatList data={courses} keyExtractor={item=>'c'+item.id} contentContainerStyle={ss.ls} renderItem={({item})=>(<View style={[ss.item,{backgroundColor:cl.cd,borderColor:cl.bd}]}><View style={{flex:1}}><Text style={[ss.it2,{color:cl.tx}]} numberOfLines={1}><FontAwesome5 name="book" size={12} color={cl.pr}/> {item.title}</Text><Text style={[ss.is2,{color:cl.ts}]}>{item.professor?'Prof: '+item.professor+' • ':''}{new Date(item.created_at).toLocaleDateString('fr-FR')}</Text></View><View style={ss.ia2}><TouchableOpacity style={[ss.ib2,{backgroundColor:cl.sc}]} onPress={()=>ri('course',item.id)}><FontAwesome5 name="undo" size={12} color="#fff"/></TouchableOpacity><TouchableOpacity style={[ss.ib2,{backgroundColor:cl.dg}]} onPress={()=>di('course',item.id)}><FontAwesome5 name="trash-alt" size={12} color="#fff"/></TouchableOpacity></View></View>)} ListEmptyComponent={<View style={ss.em}><FontAwesome5 name="folder-open" size={40} color={cl.ts} style={{marginBottom:8}}/><Text style={{color:cl.ts}}>Aucun cours masqué</Text></View>}/>)}
            {tab==='data'&&(<FlatList data={notes} keyExtractor={item=>'n'+item.id} contentContainerStyle={ss.ls} renderItem={({item})=>(<View style={[ss.item,{backgroundColor:cl.cd,borderColor:cl.bd}]}><View style={{flex:1}}><Text style={[ss.it2,{color:cl.tx}]} numberOfLines={1}><FontAwesome5 name={item.type==='support'?'file-pdf':item.type==='link'?'link':'sticky-note'} size={12} color={cl.pr}/> {item.title}</Text><Text style={[ss.is2,{color:cl.ts}]}>Cours: {item.course_title||'Inconnu'}</Text></View><View style={ss.ia2}><TouchableOpacity style={[ss.ib2,{backgroundColor:cl.sc}]} onPress={()=>ri('note',item.id)}><FontAwesome5 name="undo" size={12} color="#fff"/></TouchableOpacity><TouchableOpacity style={[ss.ib2,{backgroundColor:cl.dg}]} onPress={()=>di('note',item.id)}><FontAwesome5 name="trash-alt" size={12} color="#fff"/></TouchableOpacity></View></View>)} ListEmptyComponent={<View style={ss.em}><FontAwesome5 name="folder-open" size={40} color={cl.ts} style={{marginBottom:8}}/><Text style={{color:cl.ts}}>Aucune donnée masquée</Text></View>}/>)}
        </View>
    );
}
